...
// Sound management functions
  ...
  const handleSoundFormSubmit = (e) => {
    ...
    if (!soundFormData.name.trim() || !hasFiles) {
      console.log('Validation failed: Name and at least one audio file required')
      return
    }
  ...
  const addSound = (newSoundData) => {
    ...
    console.log('🎵 ADD SOUND - Final sound object:', newSound)
  ...
  const playSound = (sound) => {
    ...
    // Determine which file to play (support for multiple files with random playback)
    let fileToPlay
    
    if (sound.files && sound.files.length > 0) {
      // Multiple files available
      console.log('Multiple files available:', sound.files.length)
      ...
      if (sound.randomPlay && sound.files.length > 1) {
        ...
        console.log('Randomly selected file:', fileToPlay.name, 'from', sound.files.length, 'files')
      } else {
        // Play first file
        fileToPlay = sound.files[0]
        console.log('Playing first file:', fileToPlay.name)
      }
    } else {
      // Backward compatibility: use single file property
      fileToPlay = { name: sound.file }
      console.log('Using single file (backward compatibility):', sound.file)
    }
    ...
    if (storedFile) {
      soundUrl = storedFile
      console.log('Using stored file from localStorage:', fileToPlay.name)
    } else {
      soundUrl = `/assets/${fileToPlay.name}`
      console.log('Using file from /assets/ directory:', fileToPlay.name)
    }
    
    console.log('Playing sound:', sound.name, 'Loop property:', sound.loop, 'from URL:', soundUrl, 'Random Play:', sound.randomPlay)
                    ...
                    >
                      <div className="flex flex-col items-center justify-center mb-2">
                        ...
                        {!editMode && sound.files && sound.files.length > 1 && (
                          <div className="absolute top-2 right-2 w-3 h-3 bg-purple-500 rounded-full" title={`${sound.files.length} files available`}></div>
                        )}
